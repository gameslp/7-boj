...
//   derive(def, state)              → { status, provisional, places, sharedFrom, withdrawn, teamPoints, board }
//
...
function deriveBracket(def, state) {
  ...
  return {
    ...
    provisional: !done,
    places: [...podium, ...rest],
    // Finał i mecz o brąz rozstrzygają czwórkę. Reszta dzieli 5. miejsce.
    sharedFrom: 4,
    board: {
      kind: 'bracket',
...
function deriveGroups(def, state) {
  ...
  return {
    ...
    // Po fazie grupowej tylko puchar rozstrzyga miejsca 1–4.
    sharedFrom: 4,
    board: {
      kind: 'groups',
      tables,
...
function deriveHeats(def, state) {
  ...
  return {
    ...
    provisional: !done,
    places,
    // Przy grze bez heatów znamy pełną kolejność. W przeciwnym razie
    // finał ustala czwórkę, a pozostali dzielą 5. miejsce.
    sharedFrom: state.straightFinal ? null : 4,
    board: {
      kind: 'heats',
